...
public class Main {
    ...
    private void searchEmployee() {
        ...
        do {
            ...
            switch (choice){
                case 1:
                    String name = Inputter.inputNonBlankStr("Enter the Employee name: ");
                    List<Employee> list = cm.searchByName(name);
                    printList(list);
                    break;
                case 2:
                    list = cm.searchTesterHighestSalary();
                    printList(list);
                    break;
                case 3:
                    String pl = Inputter.inputNonBlankStr("Enter the Developer programming language: ");
                    list = cm.searchByPL(pl);
                    printList(list);
                    break;
                default:
                    System.out.println("Returning to Main Menu!");
    ...
    private void sort() {
        List<Employee> list = cm.sortBySalaryAndName();
        printList(list);
    }
    ...
    private void printList(List<Employee> empList) {
        try {
            ...
            if (empList.isEmpty()) {
                throw new Exception("The List of Employees is empty!");
            }
            int i = 0;
            for (Employee obj : empList) {
                switch (obj.getClass().getSimpleName()) {
                    case "Tester":
                        ...
                        System.out.println(tester.toFileString());
                        break;
                    case "Developer":
                        Developer dev = (Developer)obj;
                        System.out.println(dev.toFileString());
                        break;
                    case "TeamLeader":
                        TeamLeader tl = (TeamLeader)obj;
                        System.out.println(tl.toFileString());
                        break;
                    default:
                        break;
                }
